...
const OVA_FORM_STORAGE_KEY = "ova-deploy-form-v1";

/** Fields saved to / restored from localStorage. Passwords are intentionally excluded. */
const OVA_PERSIST_FIELDS = [
  ...
  // Dynamic selects: saved here, re-applied after each fill
...
/** True while resetOvaModal is running — prevents the programmatic form.reset()
 *  from overwriting saved storage with blank values. */
  ...
  try {
    ...
    for (const f of OVA_PERSIST_FIELDS) {
      ...
      data[f.id] = f.type === "checkbox"
        ...
        : /** @type {HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement} */ (el).value;
    }
...
function loadOvaFormStorage() {
  try {
    ...
    return raw ? /** @type {Record<string,unknown>} */ (JSON.parse(raw)) : null;
  } catch (_) { return null; }
}

/** Restore text / checkbox fields immediately (select options aren't available yet). */
function restoreOvaFormStaticFields() {
  ...
  for (const f of OVA_PERSIST_FIELDS) {
    ...
    if (!el) continue;
    if (f.type === "checkbox") {
      /** @type {HTMLInputElement} */ (el).checked = Boolean(data[f.id]);
    } else if (data[f.id] != null && data[f.id] !== "") {
      /** @type {HTMLInputElement|HTMLTextAreaElement} */ (el).value = String(data[f.id]);
    }
  }
}

/** After a dynamic select has been populated with options, try to re-apply the stored value. */
...
/**
 * Prefer same-origin URLs (typically vCenter → plug-in reverse proxy) before direct
 * https://&lt;plug-in-host&gt;:… URLs. Browsers cannot ignore self-signed certs for fetch();
 ...
 * e.g. "/plugins/com.example~1.0/192.168.1.1-8443"  → "https://192.168.1.1:8443"
...
function urlsFromPluginRoot(root, kind) {
  ...
  if (!r) return urls;
  const withUi = r.endsWith("/tanzu-hub-poc-ui") ? r : `${r}/tanzu-hub-poc-ui`;
...
/**
 * @param {string[]} extraRoots — from getPluginBackendInfo
...
function candidateDeployUrls(extraRoots = []) {
   ...
   * base URL ("…/192.168.68.5-8443"). Added last so the vCenter proxy is tried first, but if
...
function fillComputeSelect(sel, compute) {
  ...
  const ph = document.createElement("option");
  ph.value = "";
  ph.textContent = "Select cluster or host…";
  sel.appendChild(ph);
...
async function postVcenterInventory(body) {
  ...
  for (const url of urls) {
    ...
    /* vCenter UI often returns plain 404 HTML without "Cannot POST" — still wrong host; try next URL. */
    if (!res.ok && canRetry && (res.status === 404 || res.status === 405) && (looksLikeExpress404 || !text.trim().startsWith("{"))) {
      log(`POST ${url} → ${res.status} — trying next inventory URL…`);
      continue;
    }
    break;
  }
...
async function loadVcPlacement(datacenterName) {
  ...
  fillStringSelect(dsSel, "Select datastore…", datastores);
  fillStringSelect(netSel, "Select port group…", networks);
...
function openOvaModal() {
  ...
  resetOvaModal();
  restoreOvaFormStaticFields();   // restore before prefill so SDK fill only runs on truly empty fields
  if (deployStep instanceof HTMLElement) deployStep.hidden = false;
  prefillVcHostIfEmpty();
  setStatus(
    "Choose an .ova or .ovf file below. vCenter and placement fields stay visible—you can fill them before or after the descriptor loads.",
  );
  modal.showModal();
  fileInput?.focus();
}
...
async function onVcConnect() {
  ...
  setStatus("Connecting to vCenter…");
  try {
    ...
    if (!res.ok) {
      ...
      log(`Inventory POST ${usedUrl} → ${res.status}: ${String(text).slice(0, 500)}`);
      let probe = "";
      try {
      ...
      setStatus(
        `${err}${hint} Confirm the plug-in server exposes POST /api/vcenter/inventory. Probe in browser: GET ${probe || "…/api/vcenter/inventory"} (expect JSON, not 404).`,
        true,
      );
      return;
    }
    ...
    fillDatacenterSelect(dcSel, "Select datacenter…", dcs);
...
document.getElementById("datacenter-input")?.addEventListener("change", () => {
  ...
  if (!v) {
    ...
    if (dsSel instanceof HTMLSelectElement) fillStringSelect(dsSel, "Select datastore…", []);
    if (netSel instanceof HTMLSelectElement) fillStringSelect(netSel, "Select port group…", []);
...
function renderNetworkRows(networks) {
  ...
  networks.forEach((net, i) => {
    ...
    if (vcNetworkChoices.length > 0) {
      ...
      const ph = document.createElement("option");
      ph.value = "";
      ph.textContent = "Select port group…";
      sel.appendChild(ph);
...
async function onOvaFileSelected() {
  ...
  setStatus("Reading OVF descriptor from package…");
  if (deployBtn instanceof HTMLButtonElement) deployBtn.disabled = true;
  if (deployStep instanceof HTMLElement) deployStep.hidden = false;
...
deployBtn?.addEventListener("click", async () => {
  ...
  log(`Deploy: start — file=${file.name} size≈${sizeMiB} MiB`, { scroll: true });

  /** Fresh FormData each attempt — a consumed body cannot be retried on another URL. */
  ...
  deployModalInfo("Resolving plug-in server for upload…");
  if (deployBtn instanceof HTMLButtonElement) deployBtn.disabled = true;
  try {
    const bb = await getPluginBackendBaseCached();
    ...
    function setProgress(phase, pct, detail) {
      ...
      if (progressBar  instanceof HTMLProgressElement) {
        ...
        if (pct != null) {
          ...
          progressBar.removeAttribute("value"); // indeterminate — shows pulsing animation
        }
      }
    ...
    setProgress("upload", 0, "Preparing…");

    /* ── XHR deploy attempt: upload + SSE streaming response ─────────── */
    /**
     * Try one URL. Returns a Promise that resolves with the 'done' SSE event
     * data object on success, or rejects.
     * err.uploadStarted=true means bytes were already sent — skip remaining URLs.
     */
    function attemptDeploy(url) {
      return new Promise((resolve, reject) => {
          ...
          for (const block of blocks) {
            for (const line of block.split("\n")) {
              ...
              if (ev.type === "line") {
                ...
                deployModalInfo(ev.message || "Deploying…");
                setProgress("deploy", null, ev.message || "ovftool running…");
        ...
        xhr.upload.onprogress = (e) => {
          ...
          if (e.lengthComputable && e.total > 0) {
            ...
            if (Math.round(lastUploadPct) % 5 === 0) {
              log(`Deploy: upload ${Math.round(lastUploadPct)}% — ${detail}`, {});
            }
          }
        };
        ...
        xhr.upload.onloadend = () => {
          ...
          if (lastUploadPct >= 99) setProgress("deploy", null, "Upload complete — ovftool deploying to vCenter…");
          log("Deploy: upload phase complete, waiting for ovftool…", { scroll: true });
        };
    ...
    for (let idx = 0; idx < urls.length; idx += 1) {
      ...
      try {
        ...
        if (looksLikeWrongServer) {
          log(`Deploy: ${url} → ${err.message} — retrying next URL…`, { error: true });
          continue;
        }
        break;
      }
    ...
    log(`Deploy: success — ${done.message || "complete"}`, { scroll: true });
    deployModalInfo(done.message || "Deploy finished — confirm the new VM in vCenter inventory.");
    ...
    if (/Network error|Failed to fetch|NetworkError|load failed|ECONNREFUSED|aborted/i.test(msg)) {
      deployModalError(
        "Could not reach the plug-in server from your browser. Prefer the vCenter-proxied URL (same origin as this UI) — the client tries it first. If you still see this: open the plug-in server URL in a new tab, accept the certificate warning, then retry. Also confirm CORS is enabled and firewalls allow the path.",
      );
    } else {
...
// ── Wire up storage listeners for the OVA deploy modal fields ────────────────
// Attach once at page load (elements are in the DOM even before the modal opens).
for (const f of OVA_PERSIST_FIELDS) {
  const el = document.getElementById(f.id);
  if (!el) continue;
